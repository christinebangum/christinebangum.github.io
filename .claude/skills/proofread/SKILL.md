---
name: proofread
description: Run the proofreading protocol on paper files. Checks grammar, typos, overflow, consistency, and academic writing quality. Produces a report without editing files.
...
Run the mandatory proofreading protocol on paper .tex files. Produces a report of all issues found WITHOUT editing any source files.

## Steps

1. **Identify files to review:**
   - If `$ARGUMENTS` is a specific filename: review that file only
   - If `$ARGUMENTS` is "all": review all .tex files in the paper
...
2. **For each file, check for:**
   ...
   **OVERFLOW:** Overfull hbox potential, content exceeding margins
...
4. **Save each report** to `quality_reports/[FILENAME]_report.md`
